...
## Chronite Points (CP) — the temporal energy

**Chronite is energy, not an item.** Every player has a Chronite Points pool (CP), regenerating over time toward a cap, that powers the entire temporal verb-space. There's no rare-item Seal, no quest-drop Chronite, no stockpiling-as-an-item-collection. The whole temporal layer runs off one publicly-visible energy pool.

**CP is displayed publicly alongside HP.** Every player's pool is visible to everyone, like a fighting-game health bar. This transforms hidden-information mechanics into a full-information reading game (closer to chess or fighting games than poker). Depth comes from interpreting intent against visible state, not from concealment.

Consequences of public CP:
- **The pressure of a full bar.** A player at CP cap is loaded for a big move; everyone watching feels the tension. Excellent for spectator drama.
- **Strategic positioning is readable.** Consistently-low CP signals reactive play. Sudden drops from cap mean *something just happened* — and observers have a window to investigate.
- **Buffs/nerfs teach themselves.** When a Temporal Sanctum boosts an ally's regen, everyone sees their bar fill faster. When Chrono-Drain hits an enemy, their bar visibly stalls.
- **Coalition dynamics gain a temporal-economy dimension.** Allies' CP indicates faction temporal capacity at a glance.
- **Anchors leak information.** Spending CP to anchor an event signals "I'm protecting something" — sometimes valuable as a deterrent, sometimes costly as a telegraph.

CP regen rate is the central balance lever. Can be buffed/nerfed by buildings, items, status effects, alliances. Regen modifications should be capped (e.g., ±50% of base) and temporary, to prevent runaway feedback loops.

---

## Signature mechanics — the temporal verb-space

All five verbs draw from the same CP pool. Costs are publicly observable through the bar drain.

### Slowdown / Speedup (continuous, sustained drain)

Apply a rate modifier to **activity timers** within a scope — buildings under construction, crops growing, units in transit, resource accumulation. While active, drains CP at a per-second rate.

**Critical: this does not warp the world clock.** Slowdown/speedup operates on the `completes_at_world_clock` of in-flight timer-driven activities, not on world_clock itself. The world clock remains globally uniform; only the *effective rate of pending activities* changes within the scope.

Example: a building has `started_at_world_clock: 8472.300, completes_at_world_clock: 8772.300` (300s duration). A 1.5× speedup applied for 60s of buff duration advances its timer by 90 effective seconds during the window — new completion ~8742.300. Saved 30s.

Multiple modifiers can stack with explicit rules (or net to a clamped multiplier). The system tracks the active modifier set per activity and recomputes effective completion when the set changes.

Visually: faster or slower animation of the same activity, automatically derived from "current world_clock vs. completes_at_world_clock with active modifiers."

Cost shape: low intensity, sustained drain. The cheap-but-persistent end of the verb-space — continuous low-pressure temporal economy warfare.

### Temporal Strike (rollback — burst spend)

Spend CP in a large burst to roll back a **time window** of events within a **scope**:
...
All events matching scope in `[now - window, now]` are voided; the affected projection is rebuilt. Events outside scope (diplomacy, distant battles) are untouched. CP cost scales with window size — tactical (30s–2min) rollbacks are affordable, catastrophic (30min+) rollbacks require near-cap CP.
...
### Force Forward (burst spend, near-cap required)

Reverses a rollback, restoring voided events to their original world-clock positions. CP cost is very high — requires the defender to have most of their bar full. This is a momentous play, not an everyday counter.

The information game preserved: the question is no longer "do they have a hidden Seal item?" but "do they have enough CP to Force Forward?" — readable from the public bar. Players track opponent CP across a match to estimate temporal threat.

Critically (unchanged from earlier design):
- **Costs of the original rollback remain paid** (no free retry for aggressor)
- **Force Forward clobbers any actions taken in the rollback's after-window** — including third parties who acted there. Generates *collateral political damage*, alliances, and grudges.

### Temporal Acceptance (the third option)

Defender voluntarily accepts a rollback in exchange for a small CP refund or temporary regen bonus. Important because without it, players sitting on high CP feel obligated to Force Forward, eliminating the bluff layer. With it, the defender has three readable options: accept (cheap), Force Forward (huge CP burn), or do nothing (lose ground and signal they may be saving CP for something else).

### Anchors (small, persistent CP cost)

Spend a small CP amount to anchor a specific event, making it immune to rollback *and* to being clobbered by Force Forward. Anchors are the defensive layer against temporal mechanics on both axes.

Because CP is visible, anchor placement is *somewhat* observable — opponents see CP spent, may deduce something was anchored, but not necessarily what. Players will develop tells around anchor timing.

### Schism (forking) — still in the verb-space, design less developed

Fork a region into a parallel branch for K turns; the forking player can act in both. Eventually collapsed, with discarded-branch resources partially refunded. Opponents in the forked region also get to act in both branches. CP cost: large burst, comparable to Strike. Less developed than the other verbs; more design work needed.

---

## The verb-space at a glance

| Verb | Cost shape | Effect | Scope |
|---|---|---|---|
| Slowdown | CP drain over duration | Reduces rate of in-flight activity timers | Region or specific activity |
| Speedup | CP drain over duration | Increases rate of in-flight activity timers | Region or specific activity |
| Temporal Strike (rollback) | Large CP burst, scales with window | Voids events in `[T-window, T]` within scope | Region + subsystem + time window |
| Force Forward | Very large CP burst, near-cap required | Reverses a rollback, voids interim actions | Targets a specific RollbackApplied |
| Anchor | Small CP cost | Protects an event from rollback / Force Forward | One specific event |
| Schism (fork) | Large CP burst | Creates parallel branch in a region | Region, K turns |

All five (six including Schism) draw from the same public CP pool. Strategic spending across the verb-space — and reading opponents' CP to anticipate theirs — is the entire competitive game.
...
Three players: Mara, Kestrel, Doru. CP bars visible to all.

1. Mara has been at near-cap CP for several minutes — everyone watching knows she's loaded. Kestrel is 2 in-game days from invading her fortified mining outpost.
2. **Strike phase:** Mara unloads a Temporal Strike on Kestrel's military buildup, scoped to 4 in-game days of unit production in their capital. Her CP bar drops to ~20%. Kestrel's army is unmade. Doru, mining the basin's edge, loses some yield as collateral (basin resource state was in scope). He's annoyed but not destroyed.
3. **Reaction window opens.** Everyone sees Kestrel's CP — which has been quietly accumulating to ~80% over the last hour. Enough for Force Forward, barely. Will they spend it?
4. **Kestrel plays Force Forward.** Their CP drops to near-zero. The rollback reverses. Their army snaps back. Mara's defensive prep from the after-window is erased. Mara's CP stays where she spent it down to (no refund). Doru's caravan move from the after-window is also erased — he's now furious, and not at Mara.
5. **The aftermath.** Kestrel's CP at 0, Mara's at 20%, Doru's full. The temporal economy is upside-down: Doru, the smallest player, now has the biggest temporal stick. Kestrel marches on Mara conventionally; Doru opens trade to Mara to spite Kestrel; possibly threatens a Slowdown on Kestrel's advancing army for ongoing pressure.

The mechanics generate diplomatic content. The publicly-visible CP layer makes every move and aftermath legible to spectators and opponents alike. This is the design payoff.
...
TemporalStrike command
  ...
  → notify affected players (Force Forward window opens)
```

### Force Forward flow

```
ForceForward command (within reaction window after RollbackApplied)
  → validate (sufficient CP for Force Forward floor, target rollback exists, within window)
...
### Timer modifier flow (Slowdown / Speedup)

```
ApplyTimerModifier command (e.g., Slowdown on region)
  → validate (sufficient CP for sustained drain, valid scope)
  → append ActivityTimerModifierApplied event(s) — aggregate-local
    on each affected activity (Site, Unit, etc.)
  → CP drain begins (separate per-tick CPSpent events on Player aggregate)
  → on duration expiry or cancellation:
    → append ActivityTimerModifierExpired / Cancelled
    → effective completion times revert
```

Key properties:
- Modifiers are **aggregate-local events** on the affected activity's aggregate (Site for buildings, Unit for movement) — not on a separate "modifiers" aggregate
- Cascading invalidation handled naturally by scope-filtered replay (rolling back a `BuildingStarted` also voids modifiers targeting it; rolling back the modifier reverts the activity's completion time)
- The world clock is unaffected; only `completes_at_world_clock` per activity recomputes against the active modifier set
...
- **During a reaction window: a per-event coordinator is authoritative.** When a player must respond to a rollback (Force Forward / accept / wait), the affected world-clock scope is frozen until they respond or the timer expires.
...
A first-class concept, not an afterthought. Used for any reactive play that affects the world ledger (Force Forward in response to Strike, and potentially others):
...
**Design payoff:** the reaction window is a dramatic competitive moment. The match pauses while everyone watches the defender's CP bar — does it drop, signalling Force Forward? This is a feature, not a cost — the public CP bar makes the decision visible to spectators in real time, and the tension is real.
...
|---|---|---|
| **Player** | HP, CP (current + cap + regen rate + active modifiers), held resources (Ore), faction state, command rate limits | Long-lived |
...
**5. Temporal Strike — touches everything.** Can't be a single transaction. Modeled as **macro-event + aggregate-local events**:
   - `RollbackRequested` synchronously validates Player (CP balance) and scope
...
- 1 mundane resource (Ore) + Chronite Points (CP) as regenerating energy pool per player
- 1 unit type
- 1 building type
- Sites as a graph of named nodes (hex grid is a v1 concern)
- Mining (tactical, DO-mediated)
- Combat (tactical, DO-mediated)
- 1 rollback mechanic (Temporal Strike — burst CP)
- 1 counter mechanic (Force Forward — large CP burst)
- 1 timer modifier mechanic (Slowdown OR Speedup — sustained CP drain; just one to prove the pattern)
- 1 anchor mechanic
- Public CP bars (alongside HP) for all players
- Continuous async play (commands resolve as they arrive)
...
2. **CP economy tuning** — base regen rate, cap, costs for each verb (Slowdown drain/s, Speedup drain/s, Strike burst cost by tier, Force Forward floor, Anchor cost, Schism cost). The central balance lever for the entire game; will require extensive playtesting.
3. **Regen modifier rules** — what buildings, items, status effects can buff/nerf CP regen. Cap on aggregate modification (±50% suggested) to prevent runaway feedback loops. Duration limits on negative effects so a player can't be permanently locked out of temporal play.
4. **Timer modifier stacking** — when multiple Slowdown/Speedup effects target the same activity, do they multiply, add, or clamp to a single dominant effect? Cap on net effective rate (e.g., 0.25× to 3.0×)?
5. **Information on a failed Strike** — partial CP refund or intel about the defender when their Strike is Force-Forwarded? Probably yes — every duel should leak something so even the loser gains.
6. **Anchor economics** — how many can a player hold, what events qualify, can they decay over time, what's the CP cost curve?
...
13. **Optimistic concurrency for CP spend** — version numbers on Player aggregate? How does the system detect a player trying to spend CP they already committed elsewhere (e.g., Strike issued while a Slowdown is mid-drain)?
14. **Offline reaction windows** — game is always on; reaction windows can hit players who are asleep / logged off. Options to consider: longer windows for offline targets, push notifications, pre-set "auto-accept" preferences, or accept that being offline is competitive risk. Game design problem, not architecture.
...
16. **Schism design** — least developed verb. Branching semantics, what's visible across branches, how collapse interacts with CP costs and the public CP bar.
17. **CP visibility scope** — is *everyone's* CP visible to *everyone*, or only to players in the same conflict / region / alliance? Full-information chess vs. partial-information fog-of-war.